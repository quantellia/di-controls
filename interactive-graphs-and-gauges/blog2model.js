...
function updateModelText() {
  ...
  BLG2.modelText.innerHTML =
    "By investing " +
    "$" +
    numberWithCommas(BLG2.initialInvestment) +
    " in training that costs $" +
    numberWithCommas(BLG2.trainCostPerHour) +
    " per hour, " +
    numberWithCommas(Math.round(BLG2.totalHoursPurchased)) +
    " training hours can be purchased.  Since the average skill level of the workforce before the investment is " +
    BLG2.skillLevel +
    " (on a scale of 1-100), this can produce a skill level improvement of " +
    Math.round(BLG2.benefitOfOneTrainingHour) +
    " (as measured on an assessment test with a scale of 1-10) for every training hour purchased, resulting in a total predicted skills improvement score of " +
    Math.round(BLG2.totalBenefitsOfAllHours) +
    ".  Analysis of our historical data shows that this will avoid, on average, " +
    BLG2.reductionInProjectDelay +
    " days of project delay. Project delay days cost the company, on average, $" +
    BLG2.costOfDayDelay +
    " each.  This means that the expected benefit from the $" +
    numberWithCommas(BLG2.initialInvestment) +
    " initial investment is $" +
    numberWithCommas(BLG2.totalInvestmentBenefit) +
    ", representing a net " +
    returnOrLoss +
    " of $" +
    numberWithCommas(Math.abs(totalBenefit)) +
    ".";
}
...
function updateTotalSkillsBenefit() {
  ...
  BLG2.totalBenefitsOfAllHours =
    BLG2.benefitOfOneTrainingHour * BLG2.totalHoursPurchased;
  ...
  if (
    Math.abs(BLG2.totalBenefitsOfAllHours - BLG2.oldTotalBenefitNumber) >= 1000
  ) {
    ...
    colorOneGraphPoint(
      thousandVal,
      BLG2.skillsImpactOnProjectDelayChart,
      "yellow"
    );
  }
...
function updateReductionInProjectDelay() {
  ...
  //        var chart = $('#benefitOfTrainingHourDiv').highcharts();
  BLG2.reductionInProjectDelay = Math.round(
    getYValue(chart, 0, BLG2.totalBenefitsOfAllHours)
  );
...
function calculateTotalInvestmentBenefit() {
  ...
  BLG2.totalInvestmentBenefit =
    BLG2.reductionInProjectDelay * BLG2.costOfDayDelay;
...
function init() {
  ...
  colorOneGraphPoint(0, BLG2.skillsImpactOnProjectDelayChart, "yellow");
  i.value = 100;
  i2.value = 50;
  i3.value = 3000;
  i4.value = 1000;
