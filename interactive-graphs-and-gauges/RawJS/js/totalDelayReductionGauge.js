...
//                [0.9, '#DF5353'] // red
    ...
    // The speed gauge
    BLG2.totalDelayReductionChart = $('#totalDelayReductionGauge').highcharts(
      Highcharts.merge(
        gaugeOptions, {
            title: {
                text: "Reduction in project delay",
                style: {
                    color: 'purple',
                    fontSize: '10px'
                    //fontWeight: 'bold'
            ...
            series: [{
                ...
                format: '<div style="text-align:center"><span style="font-size:25px;color:' +
                    ((Highcharts.theme && Highcharts.theme.contrastTextColor) || 'black') + '">{point.y:,.0f}</span><br/>' +
                       '<span style="font-size:12px;color:silver">days</br>faster</span></div>'
                //format: '{point.y:,.0f}'
                },
                tooltip: {
                    valueSuffix: ' days'
